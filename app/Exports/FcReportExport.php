...
use DB;
class FcReportExport implements FromView,WithColumnFormatting
{
    protected $id;
    ...
    {
            $fc_visit =  DailyTherapistVisitTransection::selectRaw("daily_therapist_visit_transection.*,tt.treatment_type_title,mt.type_name,pt.visit_type,pt.starting_date,pt.visit_time,group_concat(distinct mt.type_name) as thearpy_list,group_concat(distinct daily_therapist_visit_transection.therapy_id) as thearpy_ids,concat(COALESCE(p.first_name,''),' ',COALESCE(p.last_name,'')) as patient_name,mv.visit_type_title")
            ...
            ->whereRaw('pt.treatement_status = 2 and pt.is_assigned= 0 and daily_therapist_visit_transection.is_attend = 1 and DATE_FORMAT(daily_therapist_visit_transection.treatment_date, "%m") = "'.$this->month.'" and DATE_FORMAT(daily_therapist_visit_transection.treatment_date, "%Y") = "'.$this->year.'"')
        ...
        return view('admin.reports.fc_report.export', [
            'fc_visit' => $fc_visit,
            'month' => $this->month,
            'year' => $this->year,
