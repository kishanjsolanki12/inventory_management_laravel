...
use DB;
class FuReportExport implements FromView,WithColumnFormatting
{
    protected $id;
    ...
    {
            $fu_visit =  DailyTherapistVisitTransection::selectRaw("daily_therapist_visit_transection.*,tt.treatment_type_title,mt.type_name,pt.visit_type,pt.starting_date,pt.visit_time,group_concat(distinct mt.type_name) as thearpy_list,group_concat(distinct daily_therapist_visit_transection.therapy_id) as thearpy_ids,concat(COALESCE(p.first_name,''),' ',COALESCE(p.last_name,'')) as patient_name,mv.visit_type_title")
        ...
        return view('admin.reports.fu_report.export', [
            'fu_visit' => $fu_visit,
            'month' => $this->month,
            'year' => $this->year,
