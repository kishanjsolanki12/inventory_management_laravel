  ...
  public function account_delete(Request $request)
    {
        ...
        Wishlist::where('id',$user_id)->delete();
        AddToCart::where('id',$user_id)->delete();
        AddAddress::where('id',$user_id)->delete();
...
public function profile(Request $request)
{
  ...
  public function update_profile(Request $request)
  {
    ...
    public function list_of_review(Request $request)
    {
       ...
       if(!empty($product_id))
       {
        ...
        // prd($data1);
                ...
                // $data = URL('products_image/p.product_image');
                // prd($data);
    ...
    public function add_to_cart(Request $request)
    {
      ...
      $add_to_cart = AddToCart::where('product_id',$request->product_id)->where('user_id',$user_id)->first();

      $cart_qty = !empty($add_to_cart->qty)?$add_to_cart->qty:'0';
      if(!empty($add_to_cart))
      {
         AddToCart::where('product_id',$request->product_id)->where('user_id',$user_id)->update([
  ...
  //  ]);
    ...
    public function delete_address(Request $request)
    {
      ...
      {
        AddAddress::where('user_id',$user_id)->where('id',$address_id)->update( array('deleted_at' => date('Y-m-d H:i:s'))); 

        AddAddress::where('user_id',$user_id)->where('id',$address_id)->update( array('primary_address'=>'0','deleted_at' => date('Y-m-d H:i:s'))); 
        $new_address = AddAddress::where('user_id',$user_id)->where('deleted_at',NULL)->first(); 
        if(!empty($new_address))
        {
              AddAddress::where('user_id',$user_id)->update(['primary_address' => '0']);  
  
              AddAddress::where('id',$new_address->id)->update([
      ...
      if(!empty($user_id))
      {
        ...
        if(!empty($address))
        {
          ...
          foreach($address as $raw)
          {
            foreach ($raw as $key => $value) 
            {  
              $data['user_address'][$i][$key] = !empty($value)?$value:"";    
            }
            $i++;
          }  
    ...
    public function order_manager(Request $request)
    {
       ...
       if(!empty($product_id))
       {
        foreach($product_id as $pid => $pqty)
        {
           AddToCart::where('product_id',$pid)->where('user_id',$user_id)->update([          
          'qty' => $pqty]);
        }
       }
      ...
      // order place
            ...
            if(empty($addtocard[0]))
            {
              return response()->json([ 'status' => 'error', 'code' => 401, "message" => "Your cart is empty.Add some product to completed order."]); 
            }
    ...
    public function order_listing(Request $request)
    {
      ...
      if(!empty($user_id))
      {
        $data1 = OrderManager::selectRaw("order_management.*,CONCAT(u.first_name,' ', u.last_name) as name")
                ->leftjoin('users as u','order_management.delivered_user_id', '=','u.id')
                ->orderby('id','desc')
    ...
    public function apply_promocode(Request $request)
    {
      ...
      if(!empty($data1))
      {
        ...
        foreach($data1 as $raw)
        {
          ...
          $minimum_order_value = $raw['minimum_order_value'];
          // prd($p_value);
          foreach($raw as $key => $value) 
          {
          ...
          if($data[$i]['amount'] < $minimum_order_value)
          {
            $more_order_value = $minimum_order_value - $amount;
             return response()->json([ 'status' => 'error', 'code' => 401, "message" => "Add items worth $".$more_order_value." more to avail this offer.Minimum order worth is $".$minimum_order_value."."]);
          }
          else
          {
             return response()->json([ 'status' => 'success', 'code' => 200, "data" =>$data,'message'=>'Coupon applied successfully.']);
          }
  ...
  // prd($user_id);
    ...
    public function order_status(Request $request)
    {
      ...
      $address_id = OrderManager::selectRaw("order_management.*")->where('id',$orderId)->orderby('id','desc')->first(); 
       if(empty($address_id ))
       {
        return response()->json([ 'error' => 'success', 'code' => 401, "message" =>'Invalid order']);
       }
      ...
      $product_detail = OrderManagementTransection::selectRaw("pm.id,order_management_transection.product_price as discount_price,pm.product_weight,pm.product_name,pm.product_image,order_management_transection.discount as mrp_price,pm.product_desc,order_management_transection.product_qty")
        ->leftjoin('product_management as pm','order_management_transection.product_id', '=','pm.id')

        ->where('order_management_transection.created_by',$user_id)


        ->where('order_management_transection.order_id',$orderId)
        ->orderby('order_management_transection.product_id','desc')
        ->groupBy('order_management_transection.product_id')
        ->get()->toArray();
// prd($product_detail);
