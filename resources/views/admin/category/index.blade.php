            ...
            <div class="table-responsive">
              <table id="complex_header" class="table table-striped table-bordered display" style="width:100%">
               <thead>
                        ...
                        <!-- <th class="px-4 py-3 sorting" data-sorting_type="asc" data-column_name="category_name" style="cursor: pointer">Vendor Name</th> -->
                            <th class="px-4 py-3 sorting" data-sorting_type="asc" data-column_name="category_name" style="cursor: pointer">Category Name</th>
                          <?php  if(!Auth::user()->hasRole('Vendor')) { ?>
                            <th class="px-4 py-3">Vendor Name</th>
                            <?php } ?>
                            <th class="px-4 py-3">Parent Catgeory</th>
                            <th class="px-4 py-3">Category Image</th>
                            <th class="px-4 py-3">Action</th>
                        </tr>
                    </thead>
                    <tbody>
                    @include('admin.category.user_pagination_data')
                    </tbody>
              </table>
...
$(document).ready(function(){
 ...
 {
  $.ajax({
   url:"/admin/category/fetch_data?page="+page+"&sortby="+sort_by+"&sorttype="+sort_type+"&query="+query+"&per_page="+per_page,
 ...
 $(document).on('click', '.pagination a', function(event){
  ...
  $(document).on('click', '#reset_filter', function(){
   ...
   $('#from_date').val('');
   $('#to_date').val('');
   $('#hidden_column_name').val('category.id');
